  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-[0_12px_30px_rgba(15,23,42,0.08)] sm:p-6">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <aside className="rounded-2xl border border-slate-200 bg-[#fafbff] p-4">
          <p className="text-sm font-semibold text-slate-900">New to TeamTide?</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            Learn how to make the most of collaboration and planning for your team events.
          </p>
          <button
            type="button"
            className="mt-4 inline-flex items-center gap-1 rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700"
          >
            Take a quick tour
...
function StatCard({
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-[#fafbff] px-4 py-3">
      <div className="mb-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-indigo-100 text-indigo-700">{icon}</div>
      <p className="text-2xl font-bold leading-none text-slate-900">{value}</p>
      <p className="mt-1 text-xs text-slate-500">{label}</p>
      <p className="mt-2 text-[11px] font-semibold text-emerald-600">{delta}</p>
    </article>
  );
}
